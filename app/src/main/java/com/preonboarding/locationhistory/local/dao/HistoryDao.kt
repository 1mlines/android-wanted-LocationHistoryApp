...
interface HistoryDao {
    ...
    //distance는 util.Converter.distanceConverter로 계산 가능
    @Query("SELECT DISTINCT latitude, longitude FROM history WHERE :distance <= 0.3")
    ...
    */
    @Query("SELECT * FROM history WHERE :distance <= 0.3 AND created_at = :createdAt")
